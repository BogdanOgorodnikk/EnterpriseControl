                ...
                <transition name="form-fade">
                    <form class="create-order__form" v-if="showForm" @submit.prevent="createOrder">
                        ...
                        <select class="create-order__form--select" v-model="opt_price">
                            <option v-for="opt_price in opt_prices" :key="opt_price.id" :value="opt_price.number">{{opt_price.number}}</option>
                        </select>
...
export default {
    ...
    methods: {
        async getordersInfo() {
            try {
                const result = await axios.get(`/api/ordersinfo/${this.$route.params.id}`,{
