    ...
    <div class="table-pith" v-else>
        <div class="container">
            ...
            <div class="table-pith__sum">
                Сума залишків клієнта: <span class="table-pith__sum--span">{{sumPith[0].sumPith}}</span>
            </div>
            ...
            <div class="table-pith__box-body" v-for="(pith, index) in piths" :key="pith.id">
                ...
                <div class="table-pith__item-body"  v-if="user.role_id == 1">
                    <span class="table-pith__item-body--responsive">Дата створення: </span>{{pith.data_create}}
                </div>
                <div class="table-pith__item-body">
...
export default {
    ...
    methods: {
        async getPiths() {
            try {
                const result = await axios.get(`/api/pith/${this.$route.params.id}`, {
