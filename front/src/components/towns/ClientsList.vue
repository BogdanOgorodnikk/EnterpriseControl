        ...
        <div class="container">
            ...
            <div class="clients-list__box-body" v-for="client in clients" :key="client.id">
                ...
                <div class="clients-list__item-body" v-if="user.role_id == 1 || user.role_id == 3">
                    <span class="clients-list__item-body--responsive">Борг клієнта: </span>{{client.sumDebt}}
                </div>
                <div class="clients-list__item-body" v-if="user.role_id == 1 || user.role_id == 3">
                    <span class="clients-list__item-body--responsive">Сума залишків клієнта: </span>{{client.sumPith}}
                </div>
                <div class="clients-list__item-body" v-if="user.role_id == 1 || user.role_id == 3">
                    <span class="clients-list__item-body--responsive">Сума проданого товару: </span>{{client.sumSeller}}
                </div>
                ...
                <div class="clients-list__item-body edit" @click.prevent="showEdit(client.id, client.name, client.phone, client.reserve_name, client.reserve_phone, client.shop_name, client.shop_street)" v-if="user.role_id == 1">
                    Редагувати
                </div>
...
export default {
    ...
    methods: {
        async getClients() {
            try {
                const result = await axios.get(`/api/client/${this.$route.params.id}`, {
